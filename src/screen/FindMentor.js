...
const FindMentor = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    // fetchArticles(Item?.UserDetail?.UserId, 3, 1, 'self', setArticleList);
  ...
  const getMentorList = async () => {
    ...
    try {
      ...
      console.log('url', url);
      if (url == 'https://vecospaceapi.nexgenov8.com/api/becomementor') {
        setStatus('BecomeMentor');
      }
      if (url == 'https://vecospaceapi.nexgenov8.com/api/findmentor') {
      ...
      <TouchableOpacity
        ...
        item.IsRequestSent === 'Accepted' ? null : (
          <View
            ...
            //style={{flex: item?.IsRequestSent == 'Pending' ? 0.25 : 0.3}}
          >
            {/* <TouchableOpacity
  ...
  const renderItemChat = ({item}) => {
    return (
      <TouchableOpacity
        onPress={
          () => navigation.navigate('ChatDetails', {Item: item, Type: 'Mentor'})
          // navigation.navigate('MentorChat', {Item: item, Type: 'Mentor'})
        }
        style={{
          backgroundColor: colors.textinputBackgroundcolor,
          margin: 5,
  ...
  return (
    <SafeAreaView
      ...
      <Header
        title={status == 'BecomeMentor' ? 'Become A Mentor' : 'Find Mentor'}
